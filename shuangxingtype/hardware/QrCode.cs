...
namespace doublestartyre.hardware
{
    class QrCode
    {
        ...
        public delegate void receiveMsghandle(string myBytestr);
        public event receiveMsghandle receiveEvent;
        public byte[] readBuffer = new byte[4];
        public int agvSendNum = 0;

        /// <summary>
        /// 收到正常数据后的委托和事件
        /// </summary>
        /// <param name="sendStr"></param>
        /// <param name="remotName"></param>
        /// <returns></returns>
        public delegate void CodeArrivalhandle(byte[] data);
        ...
        private string remoteHost; // "172.16.0.220"
        ...
        public QrCode(string strIp, int iPort)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    if (bytesRec > 0)
                    {
                        ...
                        {
                            byteStr += readBuffer[i].ToString() + ",";
                        }
                        byteStr += "\r\n";
                        if (receiveEvent != null)
                        {
                            receiveEvent(byteStr);
                        }
                    }
                }
                catch (SocketException se)
                {
                    SockErrorStr = se.Message;
                }
            }
        }
        ...
        private void socketDataArrivalHandler(byte[] data)
        {
                ...
                CodeArrivalEvent(data);
            
            //else
            //{
            //    strCode = System.Text.Encoding.ASCII.GetString(data);0
            //    strCode = "999999";
            //}
            //string msg;
            //if (strCode.Contains("999999"))
            //{
            //    msg = "999999";
            //    if (CodeArrivalEvent != null)
            //    {
            //        CodeArrivalEvent(msg);
            //    }
            //}
            //else
            //{
        ...
        /// <returns></returns>
        public bool SendData(byte[] sendByte)
        {
            bool result = false;
